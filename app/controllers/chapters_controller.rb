  ...
  def index
    ...
    @node.chapters.each do |chapter|
      ...
      chapter.awsdocuments.each do |document|
        tree << {title: document.title, doc_id: document.id, parent: document.chapter_id, document: true, type: document.content.file.content_type, size: document.content.file.size} if !document.archived 
      end
    end
