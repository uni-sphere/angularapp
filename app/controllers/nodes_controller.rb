class NodesController < ApplicationController
  ...
  def create
    ...
    if node.save and report.save
      if parent.chapters.where(archived: false).count > 1
        ...
        Action.create(type: 'created', object_id: node.id, object_type: 'node', object: node.name)
        ...
        if chapter.save
          ...
          Action.create(type: 'created', error: true, object_type: 'node')
  ...
  def destroy
    if @current_node.parent_id != 0 and @current_organization.nodes.where(archived: false).count > 2
      Action.create(type: 'archived', object_id: @current_node.id, object_type: 'node', object: @current_node.name)
      ...
      Action.create(type: 'destroyed', error: true, object_type: 'node')
