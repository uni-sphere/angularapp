  ...
  def invite
    if params[:emails]
      ...
      emails.each do |email|
        psw = random_password
        if user = User.invite!(email: email, name: email.split('@').first, provider: 'email', organization_id: current_organization.id, password: psw, password_confirmation: psw, skip_invitation: true)
