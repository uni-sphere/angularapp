  ...
  def invite
    ...
    if user.save
      ...
      if link.save
        ...
        Action.create(name: 'created', object_id: @current_organization.users.last.id, object_type: 'user', object: params[:email])
        render json: {success: true, user: user}.to_json, success: 200
      else
        Action.create(name: 'created', error: true, object_type: 'user')
