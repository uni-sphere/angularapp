(function () {
  ...
  leftTree.$inject = ['$rootScope', 'ipCookie', '$timeout', 'Restangular', 'Notification', 'ModalService', 'makeNestedService'];
  function leftTree($rootScope, ipCookie, $timeout, Restangular, Notification, ModalService, makeNestedService){
    var directive = {
      link: link,
      scope:{
        nodeEnd: '=',
    ...
    function link(scope){
      ...
      function render(branch){
        ...
        scope.width = d3.select($('#view-tree')[0])[0][0].offsetWidth - margin.right - margin.left;
        scope.height = d3.select($('#view-tree')[0])[0][0].offsetHeight - margin.top - margin.bottom;
