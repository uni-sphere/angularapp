(function () {
  ...
  uploadCtrl.$inject = ['$scope', 'Restangular', 'Notification', 'ipCookie', 'activateSpinner', '$upload', 'stopSpinner']
  function uploadCtrl($scope, Restangular, Notification, ipCookie, activateSpinner, $upload, stopSpinner){
        ...
        for (var i = 0; i < files.length; i++) {
          ...
          $upload.upload({
            ...
            if(numberItems == files.length){
              if(nodeDocData.id != 0){
                if(dragAndDrop || !$scope.activeChapter.$node$scope.collapsed ){
